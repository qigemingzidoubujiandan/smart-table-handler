...
import com.ruoyi.project.system.parse.parse.rule.TableParseRule;
...
@Slf4j
@Data
public abstract class AbstractTableExtractor implements TableParseRule {
    ...
    private int extractTableCell(Table table, List<Function<String, String>> thFunc,
        ...
        // 验证表格列数
        if (texts.length != this.condition().length) {
            return ExtractRet.FAIL;
        }
        // 验证表头 -> 多页的表格第一次才验证
        if (isVerifyTableTitle) {
            Object[] titles = Arrays.stream(this.condition()).map(String::trim).toArray();
     ...
     * 合并th相同的表格
     */
    protected static void mergeTableByThEqual(List<Table> tableList) {
        Table preTable = null;
