...
import com.ruoyi.project.system.parse.parse.rule.ListRule;
import com.ruoyi.project.system.parse.parse.rule.MapRule;
...
public class DemoTableExtractor extends TableExtractor {
    ...
    @Getter
    private final ListRule listRule;
    @Getter
    private final MapRule mapRule;

    public DemoTableExtractor() {
        listRule = new ListRule(ASSETS_INFO_TITLE_CONDITIONS_1);
        mapRule = new MapRule(REPORT_INFO_CONDITIONS);
    }

    @Override
    public Void extract(List<Table> tableList) {
        extractPDF_KV(tableList, mapRule);
        fuzzyMatchingExtractTable(tableList, listRule);
        return null;
    }
}
