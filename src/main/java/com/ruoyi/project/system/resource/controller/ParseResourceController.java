...
public class ParseResourceController extends BaseController {
     ...
     * 导出解析配置结果
     */
    @PostMapping("/exportParse")
    @ResponseBody
    public AjaxResult exportParse(ParseResource parseResource, HttpServletResponse response) throws Exception {
        List<ParseResource> parseResources = parseResourceService.selectParseResourceList(parseResource);
//        ParseResource parseResource1 = parseResources.get(0);
        List<String> titles = parseConfigService.getTitles(9L);
        List<Map<String, Object>> headers = convertTitlesToHeaders(titles);
        List<List<String>> content = parseConfigService.getContent(9L);
        List<List<Object>> dataList = formatData(content);
        // 动态生成类
        String dynamicClassName = "DynamicExportClass" + "_parseResource_" + parseResource.getResourceId();
        ...
        return util.exportExcel(typedEntityList, "SheetName", "Title");
    }
